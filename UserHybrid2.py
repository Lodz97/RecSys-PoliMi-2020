...
if __name__ == '__main__':
    ...
    p3alpha.fit(**{"topK": 523, "alpha": 0.7837521749729348, "normalize_similarity": False})

    itemKNNCF = ItemKNNCFRecommender.ItemKNNCFRecommender(URM_train)
    itemKNNCF.fit(**{"topK": 256, "shrink": 990, "similarity": "asymmetric", "normalize": True,
                     "asymmetric_alpha": 0.01621858527459841, "feature_weighting": "TF-IDF"})

    userKNNCF = UserKNNCFRecommender.UserKNNCFRecommender(URM_train)
    userKNNCF.fit(**{"topK": 183, "shrink": 537, "similarity": "asymmetric", "normalize": True,
                     "asymmetric_alpha": 0.3685946914809231, "feature_weighting": "TF-IDF"})
    ...
    hyb = ItemKNNScoresHybridRecommender.ItemKNNScoresHybridRecommender(URM_train, itemKNNCF, itemKNNCBF)
    hyb.fit(alpha=0.5)

    # Kaggle MAP 0.084
    hyb2 = ItemKNNScoresHybridRecommender.ItemKNNScoresHybridRecommender(URM_train, itemKNNCBF, userKNNCF)
    hyb2.fit(alpha=0.5)
